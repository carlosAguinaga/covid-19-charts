export async function getData(strCoutry) {
  const responseConfirmed = await fetch(
    `https://api.covid19api.com/total/dayone/country/${strCoutry}/status/confirmed`
  );
  const responseDeaths = await fetch(
    `https://api.covid19api.com/total/dayone/country/${strCoutry}/status/deaths`
  );
  const responseRecovered = await fetch(
    `https://api.covid19api.com/total/dayone/country/${strCoutry}/status/recovered`
  );
